...
from ._compat import reraise, imap
...
class ReferenceCounter(object):
    ...
    def remove_reference(self):
        ...
        if self._reference == 0:
            ...
            thrown = None
            for callback in itertools.chain(self._zero_refcount_callbacks,
                    imap(lambda callback: (lambda refcount: callback.remove_reference()), self._depends_on)):
                try:
                    callback(self)
